...
# Global worker loop for Multiprocessing Workers only
worker_loop = None


def init_worker():
    """Per-process initializer: set up an event loop for httpx/asyncio calls inside the MP worker."""
    global worker_loop
    worker_loop = asyncio.new_event_loop()
    asyncio.set_event_loop(worker_loop)


async def _precompile_grader(
    ...
    Refactored to run purely async on the main event loop to avoid httpx threading issues.
    """
    pre_dir = f"/home/aficek/software/storage/data/icpc_25/icpc_pre_{problem_name}_{os.getpid()}"
    ...
    # Important: This now awaits directly on the main loop
    await sandbox.execute_code(
        f"cd {pre_dir} && ./compile.sh || true", 
        language="shell", 
        timeout=120
    )
...
def run_test_case(task_args: dict, worker_id: int) -> dict:
    ...
    unique_dir = f"/home/aficek/software/storage/data/icpc_25/icpc_run_{worker_id}_{os.getpid()}_{time.time_ns()}"
    ...
    try:
        ...
        # 2. Compile only the problem solution
        compile_command = f"cd {unique_dir} && ./compile.sh"
        
        # Worker process creates its own sandbox instance
        sandbox = LocalSandbox()
        
        # Use the worker-specific loop
...
class ICPCEvaluator(BaseEvaluator):
    ...
    async def _initialize_runtime(self):
        ...
        # Initialize Sandbox on the MAIN LOOP to avoid thread-mismatched httpx clients
        self.sandbox = LocalSandbox()
        # Explicitly tag ownership for debugging, though we will rely on main loop
        self.sandbox._owner_tid = threading.get_ident()

        # Run blocking setup (file loading, pool creation) in a background thread
        def _blocking_setup():
            ...
            # Pool is created here; init_worker runs in the *worker processes*
            pool_local = multiprocessing.Pool(
                processes=self.eval_cfg.test_batch_size,
                initializer=init_worker,
            )

            return metadata_local, input_local, pool_local

        self.metadata, self.inputdata, self.pool = await asyncio.to_thread(_blocking_setup)
    ...
    async def _evaluate_entry(self, entry: dict) -> dict:
        ...
        if pid not in self.precompiled_cache:
            # FIX: Call the async precompiler directly (on main loop)
            # Do NOT use asyncio.to_thread here
            self.precompiled_cache[pid] = await _precompile_grader(
                pid,
                grader_files,
                compile_code,
                run_code,
        ...
        for i in range(0, len(all_tests), batch_size):
            ...
            # This is OK to be in a thread because self.pool.starmap is synchronous/blocking
            # and runs purely in the Multiprocessing workers (not using the main sandbox)
